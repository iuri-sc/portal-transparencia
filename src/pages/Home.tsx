...
function MetricCard({ label, value, sub }: MetricCardProps) {
    return (
        <div className="metric-card">
            ...
            <span className="font-display text-2xl text-gray-900 dark:text-gray-50 leading-none">{value}</span>
...
function ProgressBar({ value, max }: ProgressBarProps) {
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-[11px] text-gray-400 w-7 text-right">{pct}%</span>
        </div>
    )
}
...
export default function Home() {
    ...
    if (loading) return <div className="page-wrapper"><LoadingSpinner text="Carregando painel municipal..." /></div>
    ...
    return (
        <main className="page-wrapper animate-in">
            ...
            <div className="mb-8">
                <div className="flex items-end justify-between flex-wrap gap-4 mb-1">
                    <div>
                        ...
                        <h1 className="font-display font-normal text-gray-900 dark:text-gray-50" style={{ fontSize: "clamp(28px,4vw,42px)" }}>
                            Painel de Transparência
                        </h1>
                    </div>
                    <div className="flex gap-2.5">
                        <Link to="/dados" className="btn-primary">Ver projetos</Link>
                        <Link to="/despesas" className="btn-secondary">Ver despesas</Link>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
                <MetricCard label="Total Projetos" value={fmt(projetos.length)} sub={`${ativos} em andamento`} />
                <MetricCard label="Orçamento Total" value={fmtR(totalOrc)} sub={`${Math.round((totalGasto / totalOrc) * 100)}% executado`} />
                ...
                <MetricCard label="Concluídos" value={fmt(concluidos)} sub={`${projetos.filter(p => p.status === "Planejado").length} planejados`} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <section className="card p-6 lg:col-span-2">
                    <div className="flex items-center justify-between mb-5">
                        <h2 className="font-display font-normal text-xl text-gray-900 dark:text-gray-50">Projetos em andamento</h2>
                        <Link to="/dados" className="font-mono text-xs text-green-500 hover:underline">Ver todos</Link>        
                    </div>
                        ...
                        {destaque.map(p => (
                            <div key={p.id} className="py-4">
                                <div className="flex items-start justify-between gap-3 mb-2">
                                    <div>
                                        <p className="text-sm font-medium text-gray-800 dark:text-gray-200 mb-0.5">{p.nome}</p>
                ...
                <aside className="flex flex-col gap-4">
                    <section className="card p-5">
                        <h2 className="font-display font-normal text-lg text-gray-900 dark:text-gray-50 mb-4">Por Área</h2>
                        <div className="flex flex-col gap-3">
                            {(Object.entries(cats) as [string, number][]).sort((a, b) => b[1] - a[1]).map(([cat, count]) => (
                                <div key={cat}>
                                    <div className="flex justify-between text-xs mb-1">
                                        <span className="text-gray-600 dark:text-gray-400">{cat}</span>
                                        <span className="font-mono text-gray-400">{count}</span>
                                    </div>
                                    <div className="h-0.5 bg-gray-100 dark:bg-gray-800 rounded overflow-hidden">
                                        <div className="h-full bg-green-500 rounded" style={{ width: `${(count / projetos.length) * 100}` }} />
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <section className="card p-5">
                        <h2 className="font-display font-normal text-lg text-gray-900 dark:text-gray-50 mb-4">Dados do Município</h2>
                        <div className="flex flex-col gap-2.5">
                            {(
                                ...
                                <div key={k} className="flex justify-between items-baseline">
                                    ...
                                    <span className="font-mono text-sm font-medium text-gray-700 dark:text-gray-300">{v}</span>
                                </div>
                            ))}
                        </div>
                    </section>
