...
const CATS = ["Todos", "Pessoal", "Saúde", "Educação", "Infraestrutura", "Assistência Social", "Meio Ambiente", "Administração"]
...
export default function Despesas() {
    ...
    if (loading) return <div className="page-wrapper"><LoadingSpinner text="Carregando despesas..." /></div>
    if (error) return <div className="page-wrapper"><ErrorMessage message={error} onRetry={carregar} /></div>
    ...
    return (
        <main className="page-wrapper animate-in">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
                {(
                    [
                        ...
                        ["Pagos", fmt(despesas.filter(d => d.status === "Pago").reduce((s, d) => s + d.valor, 0)), `${despesas.filter(d => d.status === "Pago").length} empenhos`],
                        ["Em Processamento", fmt(despesas.filter(d => d.status === "Em processamento").reduce((s, d) => s + d.valor, 0)), "aguardando liquidação"]
                    ] as [string, string, string][]
                ).map(([l, v, s]) => (
                    <div key={l} className="metric-card">
                        <span className="section-label">{l}</span>
                        <span className="font-display text-xl text-gray-900 dark:text-gray-50 leading-none">{v}</span>
            ...
            <div className="card p-5 mb-5">
                <h2 className="font-display font-normal text-lg text-gray-900 dark:text-gray-50 mb-4 italic">
                    Distribuição por Área 
                </h2>
                <div className="flex flex-wrap gap-x-8 gap-y-3">
                    {(Object.entries(porCat) as [string, number][]).sort((a, b) => b[1] - a[1]).map(([c, v]) => (
                        <div key={c} className="min-w-30 flex-1">
                            <div className="flex justify-between text-xs mb-1">
                                <span className="text-gray-500 dark:text-gray-400">{c}</span>
                                <span className="font-mono text-gray-400">{Math.round((v / totalGeral) * 100)}%</span>
                            </div>
                            <div className="h-0.5 bg-gray-100 dark:bg-gray-800 rounded overflow-hiddem">
                                <div className="h-full bg-amber-400 rounded" style={{ width: `${(v / totalGeral) * 100}%` }} />
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {/* filtros */}
            <div className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-xl p-4 mb-5 flex flex-wrap gap-3 items-center">
                <input
                    type="text"
                ...
                <select value={stFiltro} onChange={e => setStFiltro(e.target.value)} className="input-field min-w-36">
                    {["Todos", "Pago", "Em processamento", "Pendente"].map(s => <option key={s}>{s}</option>)}
                </select>
                <select value={ordem} onChange={e => setOrdem(e.target.value as OrdemType)} className="input-field min-w-36">
                ...
                {(busca || cat !== "Todas" || stFiltro !== "Todos") && (
                    <button
                        onClick={() => { setBusca(""); setCat("Todas"); setStFiltro("Todos") }}
                ...
                <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
                    ...
                    {filtradas.length > 0 && (
                        <p className="font-mono text-sm text-gray-500">
                            Total filtrado: <strong className="text-gray-800 dark:text-gray-200">{fmt(total )}</strong>
                        </p>
                    )}
                </div>
                {filtradas.length === 0 ? (
                            ...
                            <tbody>
                                {filtradas.map(d => (
                                    <tr key={d.id} className="tr-hover">
                                        ...
                                        <td className="td">
                                            <span className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-500 px-2 py-0.5 rounded font-mono whitespace-nowrap">
                                                {d.categoria}
                                            </span>
                                        </td>
                                        <td className="td text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">{d.fornecedor}</td>
                                        <td className="td text-right font-mono text-xs text-gray-400 whitespace-nowrap">{d.empenho}</td>
                                        <td className="td text-right font-mono text-xs text-gray-400 whitespace-nowrap">{fmtDate(d.data)}</td>
                                        <td className="td text-right font-mono text-sm font-medium text-gray-800 dark:text-gray-200 whitespace-nowrap">
                                            {fmt(d.valor)}
                                        </td>
                                        <td className="td text-right">
                                            <span className={`badge ${ST[d.status as DespesaStatusType] ?? "bg-gray-100 text-gray-600"} whitespace-nowrap`}>
                                                {d.status}
                                            </span>
                                        </td>
                                    </tr>
